<template>
  <div>
    <div class="border rounded px-3 pt-3 pb-2 border-teal-light">
      <div class="mb-2">
        <!-- format date with PHP (see Comment Model)-->
        <!-- <p class="text-sm text-teal-dark mb-2">{{ comment.name }} - {{ comment.human_created_at }}</p> -->

        <!-- format date with JS -->
        <p class="text-sm text-teal-dark mb-2">De {{ comment.name }}, il y a {{ diffDateForHumans }}</p>

        <p v-if="edit">
          <textarea class="border w-full rounded p-5 text-pink-dark" v-model="bodyUpdate"></textarea>
        </p>

        <div v-else class="text-pink-dark">
          {{ comment.body }}
        ...
        <p v-if="edit" class="flex -mx-2 mt-2">
          <button
            type="button"
            class="mx-2 flex items-center text-sm text-teal-dark font-semibold"
            @click="cancel() "
          >
            <svg
              class="fill-current text-teal-dark h-3 mr-pxm pr-1"
          ...
          <button
            type="button"
            class="mx-2 flex items-center text-sm text-teal-dark font-semibold ml-4"
            @click="updateComment()"
          >
            <svg
              class="fill-current text-teal-dark h-3 mr-px pr-1"
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 20 20"
            >
              <path
        ...
        <p v-else class="flex -mx-2 mt-2">
          ...
          >
            <svg
              class="fill-current text-teal-dark h-2 mr-px pr-1"
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 20 20"
            >
    ...
    <div class="mt-4 pl-4 border-teal border-l-4 ml-5">
      <comment
        class="mb-4 border-teal"
